...
class Matrix3f(Matrix3):
    def __init__(self, *args, copy: bool = True):
        if len(args) == 0:
            self.data = numpy.zeros((3, 3), dtype=numpy.float32)
        elif len(args) == 1:
            m = args[0]
            ...
            raise ValueError("Invalid Matrix3f constructor arguments.")
    ...
    def __add__(self, other):
        if isinstance(other, Matrix3f):
            return Matrix3f(self.data + other.data, copy=False)
        elif isinstance(other, Number) or isinstance(other, numpy.number):
            return Matrix3f(self.data + numpy.float32(other), copy=False)
        else:
    ...
    def __rsub__(self, other):
        if isinstance(other, Number) or isinstance(other, numpy.number):
            return Matrix3f(numpy.float32(other) - self.data, copy=False)
        else:
            raise ValueError(f"Cannot subract a Matrix4f from a {type(other)}.")

    def __mul__(self, other):
        if isinstance(other, Matrix3f):
            return Matrix3f(numpy.matmul(self.data, other.data), copy=False)
        elif isinstance(other, Number) or isinstance(other, numpy.number):
            return Matrix3f(self.data * numpy.float32(other), copy=False)
...
if __name__ == "__main__":
    m0 = Matrix3f(1, 0, 0, 0, 1, 0, 0, 0, 1)
    m1 = Matrix3f(2, 0, 0, 0, 4, 0, 0, 0, 8)
    print(m0 * m1)
    print(2 * m0)
    print(m0 / 2)
    print(m1[0, 0])
